...
# ── FIX-VERDICT-CLOSURE-MINIMAL-RESTORE-01 (2026-04-30) — AC-2 addition ───────
# Brief AC-2: "the data has a cleaner read on X — that's where the analysis
# starts" is a Setup-style opener as the entire verdict (Card 1 verbatim from
# 30 Apr 2026 ~15:55 SAST, Gujarat Titans vs RCB Gold 1.72 WSB).


def test_whole_verdict_is_setup_opener_pattern_fires():
    """Card 1 verbatim — verdict whose ENTIRE content is the Setup-style opener.

    The 'data has a cleaner read on X — that's where the analysis starts' shape
    is anchored (^...$) so it only fires when it's the WHOLE verdict, not a
    ...
    hits = _check_vague_content_patterns(card1)
    assert any("data has a cleaner read on" in h for h in hits), (
        f"expected the new whole-verdict pattern to fire on Card 1; got {hits!r}"
    )


def test_whole_verdict_setup_opener_pattern_does_not_fire_in_longer_verdict():
    """Anchored pattern must NOT fire when 'data has a cleaner read on X' is
    just one clause of a longer verdict that does close with action+team+odds.
    """
    longer = (
        "The data has a cleaner read on Liverpool's recent form than on "
        "Chelsea's — that's where the analysis starts. Back Liverpool at "
        "1.97 with Supabets."
    )
    hits = _check_vague_content_patterns(longer)
    # The anchored whole-verdict pattern should NOT fire because the verdict
    # has a real closing recommendation. Other unanchored patterns are also
    # not expected to fire on this clean prose.
    assert not any("data has a cleaner read" in h for h in hits), (
        f"anchored pattern should not fire on longer verdict; got {hits!r}"
    )
