"""QA-BASELINE-23: Full Product Baseline After BUILD-QA22-FIX.
...
Regression checks:
  P0-1: Away-win edge section must show outcome team, not home team
  P0-2: No "Back" CTA when displayed EV <= 0%
  P1-1: No negative EV card in Hot Tips list
  P1-2: edge:detail broadcast line matches list card broadcast line
"""
...
# Also check legacy banned phrases from prior QA rounds
...
def extract_ev_pct(text: str) -> float | None:
    """Extract EV percentage from text like 'EV +3.2%' or 'EV -1.5%'."""
    if not text:
        return None
...
def extract_list_outcome(button_text: str) -> str:
    """Extract outcome team from list button text like 'Stellenbosch @ 2.60'."""
    if not button_text:
        return ""
    # Button text patterns: "TeamName" or "Home vs Away" or "Back Team @ odds"
...
async def run_qa():
    ...
    if ht_msg:
        ...
        # Check banned phrases in all list messages
        ...
        for idx, (parent_msg, btn) in enumerate(tip_buttons):
            ...
            for dmsg in reversed(detail_msgs):
                if dmsg.text and not dmsg.out:
                    ...
                    # Extract broadcast line from detail
                    ...
                    # Check for Back CTA button (URL type with "Back" or bet action)
    ...
    if mm_msg:
        ...
        # ── STEP 4: Open up to 8 My Matches cards ───────
        print("\n" + "=" * 70)
    ...
    # ── Save results ─────────────────────────────────────
    output_path = os.path.join(os.path.dirname(__file__), "..", "data", "qa_baseline_23_results.json")
    ...
    # ── Summary ──────────────────────────────────────────
    print("\n" + "=" * 70)
    print("QA-BASELINE-23 CAPTURE SUMMARY")
    print("=" * 70)
