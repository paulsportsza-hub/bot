...
### Rule 8 — Setup section is bookmaker-/odds-language-free (FIX-PREGEN-SETUP-PRICING-LEAK-01 — LOCKED 2026-04-25)

The Setup section in narrative polish output MUST NOT contain odds, bookmaker names, decimal probabilities, EV percentages, "implied probability", "fair value", or any pricing vocabulary. Polish prompts (`format_evidence_prompt` in `evidence_pack.py`) MUST NOT instruct Sonnet to pivot to "line movements" or "odds structure" inside Setup. Decimal numbers are allowed in Setup ONLY inside qualified metric phrases ("X.X goals per game", "X.X points per game", "X.X runs per game"). Bookmaker names, odds quotes, and EV percentages belong in The Edge section, not The Setup. The detector `_find_stale_setup_patterns` at `bot.py:16307-16338` is an absolute ban (NOT a staleness check) — relaxing it reopens the BUILD-NARRATIVE-WATERTIGHT-01 leak vector. Use `_has_stale_setup_context_claims` for time-based gating. Defensive belt: `_validate_polish` (bot.py gate 8a) runs the same detector at polish time so leaks are caught before persistence rather than after a DELETE round-trip. Regression guard: `tests/contracts/test_setup_pricing_ban.py` covers prompt instruction, validator gate, and the W82 baseline for 20 sport/league fixtures.
