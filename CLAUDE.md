...
### Rule 14 — Verdict templates are tier-aware AND holistically composed (locked 2026-04-28, FIX-NARRATIVE-BOILERPLATE-VERDICT-TEMPLATE-01; amended 2026-04-28, FIX-NARRATIVE-VOICE-COMPREHENSIVE-01)

All `_render_verdict*` helpers in `narrative_spec.py` MUST emit tier-banded analytical voice variants matching the verdict-generator skill rubric. Generic boilerplate phrasing such as "supporting indicator sit behind" is forbidden — enforced by `tests/contracts/test_verdict_boilerplate_ban.py` (regression guard). Each tier carries 3-5 MD5-deterministic variants seeded on `home_name + away_name`. Variants MUST reference fixture-specific data (team name via `outcome_label`, `odds`, `bookmaker`) and resolve at least one Risk factor per Rule 9 (Risk↔Verdict cohesion) via `_verdict_risk_clause(spec)` — even at W82 baseline time, where the polish-time gate 8c is not invoked. The `_verdict_support_line` helper emits 3 MD5-deterministic qualitative phrases per `(support_level, contradicting_signals)` branch and never re-introduces the singular pluralisation bug ("1 supporting indicator sit ...") nor any indicator-count cite. The audit grade snapshot at INV-NARRATIVE-AUDIT-FINAL-01 (C, 6.96/10) was driven primarily by 18/26 cards shipping this exact boilerplate — do not relax without monitoring evidence. The synthetic harness at `scripts/synthetic_harness_verdict_boilerplate_01.py` is the AC-7 evidence generator (4 tiers × 5 sports = 20 fixtures rendered to `reports/e2e-screenshots/FIX-NARRATIVE-BOILERPLATE-VERDICT-TEMPLATE-01-*` plus a `summary.json` carrying `overall_pass` + `boilerplate_substring_count` assertions).

**Path A holistic composition is REQUIRED for tier-aware variants** (FIX-NARRATIVE-VOICE-COMPREHENSIVE-01 amendment): segment-stitched composition (`_floor_verdict()` segment-appender pattern) is forbidden because each appended sub-segment escapes the voice-rubric and ships sibling boilerplate ("No adverse line movement — price is stable at this number" was Step 4 of the deleted appender). Variants are complete by construction. Length is 100-260 chars uniform per Rule unified-char-range; if a variant comes in under floor, that is a variant-design bug, not a runtime padding problem. The `_floor_verdict()` function was deleted in FIX-NARRATIVE-VOICE-COMPREHENSIVE-01 (2026-04-28) and DO NOT re-introduce. Variant counts post-rewrite: Diamond 4, Gold 4, Silver 4, Bronze 4 supported + 4 unsupported, pass/monitor 3 with bookmaker + 3 without — total 26 distinct variants, MD5-seeded on `home_name + away_name + action`.

### Rule 15 — Pregen writes only for matches in edge_results (locked 2026-04-28, FIX-PREGEN-EDGE-RESULTS-COUPLING-01)

`discover_pregen_targets()` in `scripts/pregenerate_narratives.py` MUST intersect its candidate set with `edge_results.match_key` (where `result IS NULL`) before any candidate reaches `narrative_cache`. Ghost-cache writes — narratives for matches with no edge_results row — fail mechanically at the deeplink resolver (`bot.py:_handle_card_deeplink`) and return `edge_no_longer_available`. INV-CORPUS-LIVE-COVERAGE-01 measured the cache:edge intersection at 8–12% pre-fix; ~90% of pregen's compute and budget was invisible to users. The `_PREGEN_WARM_COVERAGE_ALLOWLIST` constant (default `frozenset()`) in `scripts/pregenerate_narratives.py` exists as an explicit escape valve for future warm-coverage cases (e.g. league-wide preview seeding); a candidate passes the filter when EITHER its `match_key` is in unsettled `edge_results` OR its `league` value is in the allowlist. The `_load_unsettled_edge_match_keys(db_path)` helper is best-effort: any DB error returns an empty set so the pregen sweep never crashes (the filter then drops everything not in the allowlist — fail-safe, not fail-open). Auto-pipelines write to `narrative_cache` ONLY for matches that satisfy the intersection or appear in the allowlist. The fix shrinks the candidate set; cooldown logic (`_pregen_fill_last_attempt`), tier-gate, horizon, and `_PREGEN_MATCH_CAP` are unchanged. Regression guard: `tests/contracts/test_pregen_edge_results_coupling.py` (10 tests covering empty/partial/settled/allowlist/missing-table cases plus signature stability and observability log contract). `tests/contracts/test_pregen_discovery.py` was updated alongside the fix — its `_make_odds_db` helper now creates the `edge_results` table and each test that expects a candidate to surface seeds an unsettled edge row via `_seed_edge()`.

### Rule 16 — Pregen tier-priority sort (locked 2026-04-28, FIX-PREGEN-DIAMOND-PRIORITY-01)

The pregen sweep in `scripts/pregenerate_narratives.py::main()` MUST sort the in-window candidate list by `_TIER_PRIORITY` (Diamond=0, Gold=1, Silver=2, Bronze=3) ASC, then by kickoff timestamp ASC, **before** `_PREGEN_MATCH_CAP` truncation. Premium tiers MUST refresh before Bronze when the cap budget is constrained — INV-CORPUS-LIVE-COVERAGE-01 measured a 4h11m Diamond blackout (04:02:56–08:52:19 UTC) on launch day pre-fix when Bronze rows with sooner kickoffs displaced the live Diamond Everton edge. The `_TIER_PRIORITY` constant order matches the canonical tier ordering used across `GATE_MATRIX` (`scripts/data_health_check.py::_GATE_MATRIX`); regression-test-guarded. The `_kickoff_unix(target)` helper is best-effort — missing or unparseable kickoffs sort last in their tier band (large default `+inf`) and never raise. Candidate dicts carry a lowercase `tier` field from their source builder (`_edge_from_serving_tip`, `_build_baseline_edge_from_snapshot_row`, `_build_fixture_only_edge`); the sort key reads `(tier or edge_tier or "bronze").lower()` to defend against caller variation. The sort is applied AFTER the FIX-PREGEN-EDGE-RESULTS-COUPLING-01 intersection filter (Rule 15) and AFTER the horizon-window filter — same call site as the prior kickoff-only sort it replaces. The `_PREGEN_MATCH_CAP=60` constant is unchanged; this fix only changes ORDER of writes. Bronze coverage MUST remain ≥50% of unsettled Bronze rows in `edge_results` post-deploy; if Bronze starves, widen the cap (separate brief — do NOT widen here). Regression guard: `tests/contracts/test_pregen_diamond_priority.py` (17 tests covering tier ordering, within-tier kickoff ASC, GATE_MATRIX alignment, kickoff helper edge cases, and Wave 2A coupling-filter byte-identity).

### Rule 17 — Verdict body excludes betting telemetry (locked 2026-04-28, FIX-NARRATIVE-VOICE-COMPREHENSIVE-01)

The Verdict section MUST NOT contain: "+X% EV" (or any "% EV" pattern), "indicators line up", "supporting indicator" (any indicator-count cite), "line movement", "adverse movement", "price is stable", "price angle", "priced in", "the lean is", "supported by data". These belong in The Edge or The Risk where pricing/signal context is appropriate. The Verdict closes with a friend's read on the match — SA Braai Voice anchored to `verdict-generator/SKILL.md` + brand-voice canon (`reference/BRAND-BIBLE-v3.md` §08–09 + `reference/COPYWRITING-DNA.md` §6/§8). Source: brief Forbidden list (every verdict, every tier, every sport).

Enforcement (defence-in-depth):
- **Tone bands**: `_VERDICT_BANNED_TELEMETRY` (10 phrases) mirrored into all four `TONE_BANDS["{cautious|moderate|confident|strong}"]["banned"]` lists in `narrative_spec.py`. The polish-time `_validate_polish` gate consumes these and rejects any LLM-polished verdict containing the phrases.
- **W82 baseline**: `_render_verdict()` in `narrative_spec.py` is now a holistic dispatcher — every variant complete by construction, no segment-appender. The deleted `_floor_verdict()` was the prime leak vector (Step 4 emitted "No adverse line movement — price is stable at this number" verbatim).
...
### Rule 18 — Stadium / venue names allowed only when verified (locked 2026-04-30, BUILD-EVIDENCE-ENRICH-VENUE-SCOREBOARD-PROJECTION-01; supersedes 2026-04-28 absolute-ban version locked under FIX-NARRATIVE-VENUE-LEAK-01)

Venue / stadium names are forbidden in narrative output UNLESS they match `pack.venue` (case-insensitive) or appear in the canonical fallback set in `bot/data/stadiums.json`. The verified-list scanner (`narrative_spec.find_venue_leaks(text, pack)`) replaces the absolute-ban scanner once `pack.venue` is populated by `_fetch_espn_context`. CLAUDE.md Rule 2 (`CURRENT_STADIUMS` freshness) governs which name is current; this rule governs that the name in narrative output must match the verified source. Both rules apply jointly.

Enforcement model (replaces FIX-NARRATIVE-VENUE-LEAK-01 absolute-ban):
- ESPN scoreboard already exposes the venue. `scrapers/match_context_fetcher.py::_extract_scoreboard_venue` reads it. `_fetch_espn_context` injects it into `ctx["venue"]` for soccer (line 1220), rugby (line 1358), and cricket (line 1684). Pre-fix the EvidencePack discarded it; post-fix it lands on `EvidencePack.venue` and rides through to the prompt as `VENUE: {pack.venue}`.
- The verified-list scanner allows ANY match against `pack.venue` (case-insensitive substring, either direction — handles "Anfield" vs "Anfield, Liverpool"). When `pack.venue` is empty (fixtures with no ESPN coverage), every curated `stadiums.json` hit is treated as canonical fallback and ALLOWED.
- Cross-fixture inventions remain a leak: with `pack.venue = "Anfield"`, a verdict mentioning "Stamford Bridge" still fails the gate.
- Same gate-shape as manager validation against `coaches.json`: forbidden when invented, allowed when verified.

Defence-in-depth surfaces:
- **EvidencePack field**: `evidence_pack.EvidencePack.venue: str` (default `""`). Populated in `build_evidence_pack` from `ctx.get("venue")`.
- **Prompt injection**: `evidence_pack.format_evidence_prompt` emits `VENUE: {pack.venue}` below `EVIDENCE RICHNESS:` only when non-empty. Lives in the dynamic per-match block (below the EVIDENCE PACK split sentinel, alongside MATCH / COMPETITION / SPORT / EVIDENCE RICHNESS) so the cache prefix per Rule 22 is unaffected.
- **Verified-list loader + scanner**: `narrative_spec._load_banned_venues()` (cached, module-level) + `narrative_spec.find_venue_leaks(text, pack)` + `narrative_spec.validate_no_venue_leak(text, pack)`. Both helpers accept either an `EvidencePack` instance or a serialised dict (legacy callers passing no `pack` get the canonical fallback default).
- **Verdict path (Gate 9)**: `min_verdict_quality()` in `narrative_spec.py` threads `evidence_pack` through to `find_venue_leaks(text, evidence_pack)`. Logs `verdict_rejected_venue_leak: tier=X len=N venues=[...] text=...`.
- **Narrative validator (Gates 1 / 6)**: `narrative_validator._validate_narrative_for_persistence` calls `find_venue_leaks(narrative_html, evidence_pack)` and `find_venue_leaks(verdict_html, evidence_pack)`.
- **Narrative polish (Gate 8f)**: `_validate_polish()` in `bot.py` calls `find_venue_leaks(polished, evidence_pack)`. Logs `POLISH REJECT: venue-leak (...)`.
- **W82 baseline path**: deterministic templates do NOT name venues by construction; no separate gate needed.
- **Regression guard**: `tests/contracts/test_venue_leak.py` — verified-list mode test corpus (TestVerifiedVenueAllowed, TestUnverifiedVenueRejected, TestRule18FailOpen, TestStadiumsJsonFallback) plus loader-integrity + dry-run repro + Gate 9 wiring + case-insensitive matching + multi-venue dedup + clean-text false-positive guards.

DO NOT add bare single-word entries like "Stadium", "Park", "Arena", "Ground", or "Field" to `stadiums.json` — they would fire on legitimate prose ("park the bus", "the home stadium gives them an edge", "ground them out"). The list curates UNIQUE compound venue names only. False positive risk audit per addition: search the corpus for the proposed term in non-venue contexts before adding. DO NOT remove the `pack` parameter from `find_venue_leaks` / `validate_no_venue_leak` — backward-compat with legacy callers (no `pack` passed) is preserved by the canonical-fallback default, but the parameter is what makes the verified-list mode work for ESPN-fed fixtures.

### Rule 19 — AI Breakdown reader filters empty narrative_html (locked 2026-04-28, FIX-AI-BREAKDOWN-EMPTY-NARRATIVE-FILTER-01)

Three SELECTs reading `narrative_cache.narrative_html` for the breakdown surface (`card_data.py:build_ai_breakdown_data`, `bot.py:_get_cached_narrative` preferred branch, `bot.py:_get_cached_narrative` legacy fallback branch) MUST filter on `narrative_html IS NOT NULL AND LENGTH(TRIM(COALESCE(narrative_html, ''))) > 0`. Verdict-cache rows write `narrative_html=''` (empty string) by design at `bot.py:_store_verdict_cache_sync` INSERT path (lines 15619-15632) — these are NOT breakdown-eligible and the reader excludes them mechanically. The `build_ai_breakdown_data` callsite synthesizes an instant-baseline row via `narrative_spec.build_narrative_spec(...)` + `narrative_spec._render_baseline(spec)` when no eligible row exists, ensuring users ALWAYS get content (zero LLM, zero ESPN — same path as `_generate_narrative_v2(live_tap=True)`). The `_store_verdict_cache_sync` writer at `bot.py:15619-15632` is unchanged — verdict_html surface (card image) continues to serve verdict-cache rows via `_get_cached_verdict` which reads `verdict_html` directly and is exempt from this rule (the card image must continue to render even when no narrative is cached). Regression guard: `tests/contracts/test_breakdown_filter.py`.

DO NOT remove the `narrative_html IS NOT NULL AND LENGTH(TRIM(...)) > 0` filter from any of the three SELECTs — every removal reopens the FIX-AI-BREAKDOWN-EMPTY-NARRATIVE-FILTER-01 leak vector. DO NOT add the same filter to `_get_cached_verdict` (`bot.py:_get_cached_verdict`) — that reader's job is to serve `verdict_html` for the card image and is intentionally agnostic to `narrative_html` state. DO NOT modify `_store_verdict_cache_sync` to write a non-empty placeholder for `narrative_html` — the empty-string sentinel is what makes verdict-only rows mechanically distinguishable from full pregen rows.

### Rule 20 — AI Breakdown button-gate is mechanically consistent with synthesis fallback (locked 2026-04-28, FIX-AI-BREAKDOWN-BUTTON-GATE-COVERAGE-01)

`_has_any_cached_narrative(match_id)` in `bot.py` MUST return True when EITHER (a) a non-expired, non-quarantined `narrative_cache` row with non-empty `narrative_html` exists OR (b) `edge_results` has any row for `match_id` (so `card_data._synthesize_breakdown_row_from_baseline()` can produce content via `narrative_spec.build_narrative_spec(...)` + `_render_baseline(spec)`). The gate predicate MUST be a strict superset of the synthesis predicate. The bulletproof contract is: "edge exists → button shows → content renders" — Diamond users always see a real breakdown (cache w84 polish OR cache w82 baseline OR synthesized w82 baseline), lower tiers always see the locked-button upsell hook on every edge card.

This rule ensures the FIX-AI-BREAKDOWN-EMPTY-NARRATIVE-FILTER-01 fallback is REACHABLE from the user's perspective. Rule 19 made the breakdown reader skip empty-narrative rows; this rule makes the button-gate not hide on cache misses (so the user can tap and reach the fallback). Without both rules together, missing cache rows present as "no button at all" — the symptom Paul reported on launch day +1.

DO NOT narrow the predicate to only check `narrative_cache` — that reopens the FIX-AI-BREAKDOWN-BUTTON-GATE-COVERAGE-01 leak. DO NOT add a different predicate (e.g. `result IS NULL` filter on the edge_results check) without updating `_synthesize_breakdown_row_from_baseline` in lockstep — the gate-and-synthesis predicates MUST stay aligned. Regression guard: `tests/contracts/test_breakdown_filter.py` includes mechanical-consistency tests asserting the gate returns True iff synthesis can produce content.

### Rule 21 — w82 / baseline_no_edge are valid for ALL tiers including Gold/Diamond (locked 2026-04-28, FIX-PREGEN-COVERAGE-DIAMOND-01)

The Stream4 refusal at `bot.py:_store_narrative_cache` (formerly refused `narrative_source in ("w82", "baseline_no_edge")` with `edge_tier in ("gold", "diamond")` writes) and the matching serve-time `w82_for_tier:gold|diamond` quarantine gate at `bot.py:_get_cached_narrative` (formerly returned None for premium-tier w82 reads) are BOTH LIFTED. Both gates were correct in pre-launch architecture when w82 was lower quality, but the post-W80 lock-down — Rules 12 (no pricing vocab in Setup), 14 (tier-aware holistically-composed verdict variants, 26 distinct), 17 (verdict body excludes betting telemetry), 18 (no venue leaks) — ensures w82 is editorial-quality content suitable for any tier.

Pregen still attempts w84 polish first; w82 is the safety-net write when polish fails. Subsequent pregen cycles retry polish and `INSERT OR REPLACE` the row with w84 when it succeeds, so the cache evolves toward polished content over time. Premium-tier w82 writes are logged as `FIX-PREGEN-COVERAGE-DIAMOND-01 PremiumW82Write match_id=...`; premium-tier w82 serves are logged as `FIX-PREGEN-COVERAGE-DIAMOND-01 PremiumW82Serve match_id=...`. These log entries enable polish-failure-rate monitoring without affecting user delivery.

DO NOT re-introduce a tier policy at either the writer or the reader. DO NOT add a `w82_for_tier:gold|diamond` quarantine reason. The "rich then gone" defect that motivated the original gates was a pre-launch concern with thin baselines — Rules 12/14/17/18 close that vector by construction. If a future regression is suspected, file a separate INV brief rather than re-tightening these gates: synthesis-on-tap (Rule 20) is the bulletproof safety net regardless of cache state, and any cache-policy change must preserve the synthesis predicate alignment.

### Rule 22 — Polish-path cache_control prefix MUST exceed model minimum (locked 2026-04-28, FIX-PREGEN-STATIC-PREFIX-PURE-01)

The `_static` content block fed to Anthropic's `cache_control` directive (the first array element in the `messages.create` content list at `pregenerate_narratives.py:2156` Haiku non-edge preview, `:2194` Sonnet edge polish, and `:2477` Sonnet retry) MUST be ≥ 1024 tokens for Sonnet polish calls and ≥ 2048 tokens for Haiku polish calls (documented Anthropic minimums). Empirically `claude-sonnet-4-6` via the `messages.content` `cache_control` pattern requires the cached prefix to clear ~ 2200 tokens before the cache HIT path engages: at 1719 tokens the cache_control directive is accepted (cache_create > 0 on call 1) but subsequent calls with different dynamic content miss (cache_read = 0) — Anthropic appears to extend the cache key into the unmarked tail when the marked prefix is sub-threshold. Pre-fix the non-combat Sonnet polish prefix sat at 823 tokens, so the directive fired but no cache was written or read at all. Result: 0% cache hit on the Sonnet polish surface, ~$15-37/month wasted on input tokens that should have hit the cached prefix at 0.10× cost.

Adding per-match interpolation (`tone_band`, `verdict_action`, `verdict_sizing`, `bookmaker @ odds`, `_tier_key`, `evidence_class`, `h2h_guardrail`) above the cache split point at `evidence_pack.py:format_evidence_prompt` defeats the cache by injecting per-match variation into the cached prefix. The reorder pattern landed in this brief is canonical: literal-content blocks — OUTPUT FORMAT section descriptions, BANNED PHRASES (match_preview branch only), VERDICT BODY EXCLUSION, the static base of `VERDICT QUALITY CONSTRAINT` (vocabulary list + verdict-shape rules), AND the verdict-specific SA VOICE rules block — sit ABOVE the EVIDENCE PACK split sentinel inside a NEW `STYLE & OUTPUT GUIDE` block. Per-match interpolation sits BELOW the split inside a NEW `PER-MATCH CONSTRAINTS` block. The role_lines + RULES 1–11 + COMBAT-SPORT EVIDENCE LAW already sat above the split and are unchanged.

Implementation: `format_evidence_prompt(pack, spec, match_preview, return_split=False)` in `evidence_pack.py` builds two `output_guide_static_lines` variants (one per branch — match_preview and edge), inserts them between the RULES block and the EVIDENCE PACK separator, and emits the per-match-interpolation block last. `_verdict_quality_lines` was split into `_verdict_quality_static_lines` (literal vocab list + verdict-shape rules + SA VOICE block — moves above split into the edge branch's STYLE & OUTPUT GUIDE) and `_verdict_quality_dynamic_lines` (per-tier `_verdict_char_floor` line + diamond-only conditional — stays below split). The split helper at the function tail (`next(i for ... if line == _sep)`) is unchanged — its sentinel is the EVIDENCE PACK header, which still appears exactly once in the assembled `prompt_parts`. Callers MUST keep the same two-element `content` list shape: `[{"type": "text", "text": _static, "cache_control": {"type": "ephemeral", "ttl": "1h"}}, {"type": "text", "text": _dynamic}]`.

Token effect (live `messages.count_tokens` against `claude-sonnet-4-6`, all branches measured 2026-04-28 with synthetic minimal `EvidencePack`):

| Path                                             | Pre-fix | Post-fix     | Sonnet min | Clears? |
|--------------------------------------------------|--------:|-------------:|-----------:|---------|
| Sonnet edge polish, non-combat (soccer/rugby)    | 810     | 2250         | 1024       | ✓       |
| Sonnet edge polish, combat (mma/boxing)          | 1155    | 2595         | 1024       | ✓       |
| Sonnet retry, non-combat                         | 810     | 2250         | 1024       | ✓       |
| Haiku non-edge preview                           | 494     | 1658         | 2048       | ✗       |

Haiku non-edge preview remains sub-threshold (1658 < 2048) post this fix — out of scope per the brief, separate FIX brief if material. The Haiku preview path uses `match_preview=True` which renders a different (shorter) STYLE & OUTPUT GUIDE block (no SA VOICE / verdict-quality static base because preview emits no bet recommendation); padding it to ≥ 2048 would either bloat the prompt with redundant content or require switching the preview model to Sonnet (cache min 1024).

Live cache verification (AC-7 synthetic harness, 5 distinct soccer fixtures rendered against `claude-sonnet-4-6` with the production `messages.content` + `cache_control: {ephemeral, ttl: 1h}` + `anthropic-beta: extended-cache-ttl-2025-04-11` pattern): call 1 wrote 2247 tokens to cache; calls 2–5 each read 2247 tokens from cache. Total cache_creation = 2247 (one write); total cache_read = 8988 (4 reads). Achieved cache hit rate on the 5-fixture run: 4/5 = 80 %, expected to approach 95 %+ in steady-state production traffic.

Regression guard: `tests/contracts/test_polish_prefix_threshold.py` — 5 tests:
- `test_static_prefix_meets_sonnet_minimum_non_combat` — live `messages.count_tokens` against soccer fixture, asserts `static_tokens >= 1024` (skips when `ANTHROPIC_API_KEY` unset).
- `test_static_prefix_meets_sonnet_minimum_combat` — live count against MMA fixture, same assertion (combat clears 1024 pre-fix at 1168 too — guards against COMBAT-SPORT EVIDENCE LAW being dropped above the split).
- `test_split_sentinel_preserved` — structural test (no API), asserts `EVIDENCE PACK` sentinel still defines the split, `STYLE & OUTPUT GUIDE` header lives in static, `PER-MATCH CONSTRAINTS` header lives in dynamic.
- `test_static_block_carries_moved_content` — structural test, asserts the 4 named INV-G4 blocks live in static for both branches.
...
### Rule 24 — Writer-level refusal of premium-tier W82 / baseline_no_edge persistence (locked 2026-04-29, FIX-W84-PREMIUM-NO-FALLBACK-CLOSE-SAFETY-NET-01)

`_store_narrative_cache()` in `bot.py` MUST refuse to persist `narrative_source ∈ {"w82", "baseline_no_edge"}` when `edge_tier ∈ {"gold", "diamond"}`. This is the writer-level enforcement of Rule 23, closing the safety-net residual from FIX-PREGEN-COVERAGE-DIAMOND-01 that wrote premium W82 rows on every code path that bypassed the Wave 2 pregen-side intercept. The 12-hour live observation post Wave 2 deploy showed `atletico_madrid_vs_arsenal_2026-04-29` Gold edge accumulating PremiumW82Write events on a 6-minute cycle (16+ events per `journalctl --since '6 hours ago'`); user-visible regression: Liverpool-vs-Chelsea Gold AI Breakdown showed pure W82 boilerplate across all 4 sections.

**Bypass paths the refusal closes:**

- **`_skip_w84` carve-out** at `pregenerate_narratives._generate_one`. PARTIAL coverage with `_coverage_level == "empty"` for Gold/Diamond, or `edge.skip_sonnet_polish` flag, sets `_skip_w84 = True`. Wave 2 intercept gates on `not _skip_w84` — skipped. Narrative continues as `w82` with the `_cache` dict populated; the gap-fill loop (line 3169) and the main pending-writes loop (line 3604) call `_store_narrative_cache(..., narrative_source="w82", ...)`.
- **`_is_non_edge` carve-out** at `pregenerate_narratives._generate_one`. Non-edge premium-tier previews bypass the intercept by design — the chain only fires for failure-mode downgrades, not legitimate non-edge baselines. The writer-level refusal still rejects the persistence (synthesis-on-tap covers the resulting cache miss).
- **Bot serve-time persist** at `bot.py::_generate_game_tips::_persist_narrative_bg`. The call site at the end of `_generate_game_tips` invokes `_store_narrative_cache(db_match_id, msg, tips, _edge_tier, "sonnet")` without overriding the `narrative_source="w82"` keyword default. The `live_tap=True` baseline produced by `_generate_narrative_v2` is W82 by construction; the call site labels it as such and the cache row lands as `narrative_source='w82' AND edge_tier IN ('diamond','gold')`. Pre-fix this fired `PremiumW82Write` on every premium tap to a match without a pregen-cached W84 row.

**Implementation:** in `_store_narrative_cache()`, after the `_wg_tier` / `_wg_src` lowercasing block, the conditional `if _wg_src in ("w82", "baseline_no_edge") and _wg_tier in ("gold", "diamond"):` logs `FIX-W84-PREMIUM-NO-FALLBACK-CLOSE-SAFETY-NET-01 PremiumW82WriteRefused match_id=... narrative_source=... edge_tier=... verdict_len=...` at WARNING level (so polish-failure rates remain monitorable in journalctl) and then `return` — the function exits before any row is persisted. Silver and Bronze tiers continue to write W82 baselines (W93-TIER-GATE cost policy unchanged).

**Relationship to Rule 21 (read-side lift, FIX-PREGEN-COVERAGE-DIAMOND-01):** Rule 21 keeps governing the read surface — `_get_cached_narrative` continues to serve any premium W82 row that exists pre-flush, and the `w82_for_tier:gold|diamond` quarantine reason is NOT re-introduced. Post-deploy + cache invalidation (the brief's AC-7 `DELETE FROM narrative_cache WHERE narrative_source IN ('w82','baseline_no_edge') AND edge_tier IN ('diamond','gold')`) leaves the read surface with zero premium W82 rows to serve in steady state. Synthesis-on-tap (Rule 20) fills the cache miss at view time.

**Relationship to Rule 23 (Wave 2 pregen-side intercept):** Rule 23 is the canonical no-fallback chain for premium tiers (Sonnet retry → Haiku → defer). Rule 24 is the second-layer enforcement covering paths that bypass the intercept. They are complementary, not overlapping — the `_premium_intercept_tier` block at `pregenerate_narratives._generate_one` line 2587 returns BEFORE the `_cache` dict is populated when the Haiku fallback fails (no row written; defer counter incremented; EdgeOps alert at 3 consecutive defers). Rule 24 catches the cases the intercept doesn't cover.

**Regression guard:** `tests/contracts/test_premium_no_w82_invariant.py` — 7 tests:
- `test_corpus_invariant_no_premium_w82_rows` — corpus-level scan against live `narrative_cache` (skipped when DB absent).
- `test_writer_refuses_premium_w82_and_baseline_no_edge` — parametrised matrix: 2 sources × 2 tiers (Gold/Diamond × W82/baseline_no_edge) all refused (AC-5(a) + AC-5(b)).
- `test_writer_persists_silver_w82_unchanged` — Silver + W82 row persisted (AC-5(c)).
- `test_writer_persists_bronze_baseline_no_edge_unchanged` — Bronze + baseline_no_edge row persisted (AC-14 belt-and-suspenders).
- `test_writer_persists_diamond_w84_unchanged` — Diamond + W84 row persisted (positive control: refusal is source-scoped).
- `test_writer_persists_gold_haiku_fallback_unchanged` — Gold + w84-haiku-fallback row persisted (Wave 2 success path is not blocked).
- `test_brief_log_marker_present_in_writer` — source-level check: `PremiumW82WriteRefused` log marker is present and the conditional contains an early `return` (without the return the marker is logging-only).

**DO NOT** narrow the refusal to skip carve-outs (`_is_non_edge` / `_skip_w84`) — the writer doesn't know the carve-out reason, and synthesis-on-tap covers the resulting cache miss. The corpus invariant in AC-5 says ZERO premium W82 rows; including carve-outs is intentional. **DO NOT** loosen the tier scope (Silver/Bronze must continue writing W82 per W93-TIER-GATE — AC-14 is byte-identity for non-premium tiers). **DO NOT** rename the log marker `PremiumW82WriteRefused` — monitoring and AC-9 corpus-delta verification both grep for this exact string, and the soft-disable rollback path uses the marker to confirm reversion. **DO NOT** add the same refusal to `_get_cached_narrative` — that's the read surface and Rule 21 explicitly governs it. **DO NOT** route the refused write into the Wave 2 chain inline (Path B) — the writer is a defensive lib function, not a generator; routing would create a second caller into the chain logic and violate the single-source-of-truth invariant.

**Soft-disable rollback** (per the brief's rollback block): `git revert <fix-commit-sha>` restores the safety-net path. Cache will re-populate with W82 premium rows on next sweep — back to the regression. Trigger conditions for invoking rollback: pregen W84 SERVED rate drops below 80 % within 4 h post-deploy; premium-tier defer rate > 10 % per sweep (cards stuck > 2 h); user-visible regression of empty/missing breakdowns instead of polished prose; `PremiumDefer` alert volume > 5/day (chain calibration broken).
