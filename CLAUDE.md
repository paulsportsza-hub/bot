...
18. **[SO #44]** Model Routing v1 — pick the cheapest/fastest model that can safely complete the task. Two dispatch modes via `DISPATCH_MODE` env on the bridge: **`hybrid`** (default — Sonnet executes 70-80%, Codex High mechanical, Opus Max judges, Codex XHigh hard root-cause) and **`pure-codex`** (every dispatched brief routes to `codex --profile xhigh` regardless of declared agent — used when Codex is the canonical executor). Cowork orchestration sessions (LEAD/AUDITOR/COO/NARRATIVE) always run on Claude regardless of mode. Toggle: edit bridge plist `EnvironmentVariables` + `launchctl kickstart`. Canonical: [Notion](https://www.notion.so/354d9048d73c8138bf72d8ce7b768a08) · mirror: [`ops/MODEL-ROUTING.md`](ops/MODEL-ROUTING.md). (LOCKED 2 May 2026; pure-codex switch added 6 May 2026 `FIX-PURE-CODEX-DISPATCH-SWITCH-01`.)
