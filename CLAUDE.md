...
*Last updated 7 May 2026 (`OPS-CODEX-REVIEW-INLINE-MIGRATE-01`). Lock history + version bumps: [`ops/CLAUDE-CHANGELOG.md`](ops/CLAUDE-CHANGELOG.md). Latest standing orders: SO #43 (session role locks dispatch role, May 1+2), SO #45 (Codex Review Gate v4.5 + dispatch-runner clarification, 7 May), SO #46 (no time references, 4 May).*
...
19. **[SO #45]** Review Gate — every code-touching brief runs a fresh-context review after commit + push and before `mark_done.sh`. **Hybrid mode (Claude executor — interactive Cowork session only):** `/codex:review --wait` slash command — fires the codex-companion script as a fresh node subprocess, providing cross-process review. Dispatch-runner-spawned agents must use inline `codex --profile xhigh exec` even in hybrid mode (SessionEnd hook is unreliable outside interactive sessions → broker orphan risk). **Pure-codex mode (Codex executor):** plugin path is REDUNDANT (executor is already Codex; calling its own plugin adds nothing and has hung agents on background-task UX). Instead, the agent spawns a **fresh `codex exec` sub-agent inline** — synchronous, no plugin, no shared session: `codex --profile xhigh exec "<review prompt>"`. New process = fresh context = unbiased sub-agent. Reports without `## Codex Sub-Agent Review` section showing `Outcome: clean | blockers-addressed | needs-changes | bootstrap-exempt` are INCOMPLETE — brief reopens. Mandatory adversarial triggers (money/payments, auth/settlement, non-rollback-safe migrations) → adversarial framing in the sub-agent prompt. Detail + canonical sub-agent prompt: `ops/DEV-STANDARDS.md` § Codex Review Gate v4.5 + § Pure-Codex Sub-Agent Review pattern. (LOCKED 3 May 2026 v4.4; amended 4 May 2026 v4.5; pure-codex inline-sub-agent rewrite 6 May 2026 `FIX-SO45-CODEX-INLINE-SUBAGENT-01`; dispatch-runner clarification 7 May 2026 `OPS-CODEX-REVIEW-INLINE-MIGRATE-01`.)
