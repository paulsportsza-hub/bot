...
*Last updated 7 May 2026 (`DOCS-SO-RUNTIME-PATH-ISOLATION-02`). Lock history + version bumps: [`ops/CLAUDE-CHANGELOG.md`](ops/CLAUDE-CHANGELOG.md). Latest standing orders: SO #43 (session role locks dispatch role, May 1+2), SO #45 (Codex Review Gate v4.5, 4 May), SO #46 (no time references, 4 May), SO #52 (production runtime path isolation, 7 May).*
...
> 6. (card-template / glow / visual work only) [`ops/CANONICAL-GLOW-SPEC.md`](ops/CANONICAL-GLOW-SPEC.md) — locked 7 May 2026 after TWO regression cycles in 5 days. Working pattern: `.upper-section` / `.upper-glow-zone` wrapper with `overflow:hidden`, `.header { overflow: visible }`, glow divs as direct children of the WRAPPER not `.header`, anchor `at 50% 45%`, per-tier classes. Contract tests: `tests/contracts/test_match_detail_canonical.py` + `tests/contracts/test_edge_detail_canonical.py` (12 assertions, both must pass). Forbidden patterns codified — `at 50% 25%` (top-center, clips inside header) and `at 92% 50%` (right-side) both rejected by Paul.
> 7. (any monitoring / alerter / health-check / digest / arbiter / scheduled-task work) [`ops/CONTROL-TOWER.md`](ops/CONTROL-TOWER.md) → authoritative spec [`HANDOFFS/mzansiedge_daily_maintenance_control_tower.md`](HANDOFFS/mzansiedge_daily_maintenance_control_tower.md) — locked 7 May 2026 by SO #51. Daily Control Tower runs 7 Cowork scheduled tasks (06:00 → 08:10 SAST). All findings classified P0/P1/P2/P3. Telegram is exception-only. Paul receives ONE digest at 08:10 SAST. Old arbiters / `regression_arbiter.py` / `arbiter_qa.py` / `mzansiedge-daily-health-routine` are decommissioned — do not re-enable. Adding a new monitor requires reading the spec first and respecting the severity classification.
> 8. (any deployment, bot-prod update, or `deploy_bot_prod.sh` work) [`ops/RUNTIME-ISOLATION.md`](ops/RUNTIME-ISOLATION.md) — locked 7 May 2026 by SO #52. `bot-prod/` is the read-only prod checkout; `bot-data-shared/` is the writable shared volume. Deploy = `deploy_bot_prod.sh <SHA>` only. Never write directly to `bot-prod/`. Rollback = `deploy_bot_prod_rollback.sh`. CI auto-trigger disabled until T101.
...
| [`ops/CONTROL-TOWER.md`](ops/CONTROL-TOWER.md) → spec [`HANDOFFS/mzansiedge_daily_maintenance_control_tower.md`](HANDOFFS/mzansiedge_daily_maintenance_control_tower.md) | **🔒 LOCKED 7 May 2026 by SO #51.** Daily Maintenance Control Tower — 7 Cowork morning tasks (Health Gate / Runtime Canary / Edge Quality / Narrative QA / Social Safety / SEO Exception / Daily Digest). Replaces `regression_arbiter.py`, `arbiter_qa.py`, old `mzansiedge-daily-health-routine`. P0/P1/P2/P3 classification mandatory. Telegram exception-only. Paul gets ONE digest at 08:10 SAST. | Any monitoring / alerter / health-check / digest / scheduled-task work |
| [`ops/RUNTIME-ISOLATION.md`](ops/RUNTIME-ISOLATION.md) | **🔒 LOCKED 7 May 2026 by SO #52.** Production runtime path isolation — `bot-prod/` is read-only checkout at deploy SHA; `bot-data-shared/` is the writable shared volume (DBs, reports, logs). Deploy via `deploy_bot_prod.sh <SHA>`; rollback via `deploy_bot_prod_rollback.sh`. CI auto-trigger disabled (T100) until T101. Never write directly to `bot-prod/`. | Any deployment, bot-prod update, or `deploy_bot_prod.sh` work |
...
8. **[SO #30]** Line-range file ops only — no full-file reads on files >500 lines. Grep first, then targeted Read with offset/limit. Edits use Edit, never Write. **Blast-radius corollary (LOCKED 6 May 2026, supersedes 5 May rev — focuses on real risk, stops blocking normal responsible fixes):** brief authoring discipline scales with PRODUCTION-file blast radius, not raw file count. **≤3 production files:** agent may proceed with normal grep+read discovery (no pre-baked snippets required). **>3 production files:** brief AC MUST contain explicit per-file intent + OLD/NEW snippets, or pre-flight approval token. **Tests do not count toward the threshold** — UNLESS they require broad fixture / harness / framework refactors that themselves shape the production change. **Shared-behavior changes** (modifying a util, contract, or function used by ≥3 callers) require extra review even if only 1 file actually changes — declare under a `Shared-behavior:` block in the brief AC. Documented incidents that drove the original snippet rule: FIX-DBLOCK-RUNTIME-HOT-PATHS-01 (12-min thrash, 7 production files); FIX-BRIEF-AUTHORING-MULTIFILE-DISCIPLINE-01 (24-min thrash, 4 production files). (Original LOCKED 7 Apr 2026; corollary revised 6 May 2026 `FIX-SO30-BLAST-RADIUS-01`.)
...
15. **[SO #41]** Approval binds commit. Cowork lead reviewing a code/data-change report MUST verify commit landed + pushed before closing. Verification block: `git log --oneline | grep <BRIEF-ID>` (≥1 hit) + `git rev-list --left-right --count HEAD...@{upstream}` (`0\t0`). If either fails: open `OPS-COMMIT-RECOVER-<BRIEF-ID>` follow-up. Anchor: `feedback_so41_approval_binds_commit.md`. (LOCKED 25 Apr 2026.)
    - **OPS-CANONICAL-LANE-COMMIT-DISCIPLINE-01 amendment:** canonical QA gallery files under `static/qa-gallery/canonical/**` are atomic-commit-only and must not be co-staged with non-canonical files. The `.githooks/pre-commit` hook invokes `scripts/canonical_lane_check.sh` and rejects mixed staging unless an audited override is explicitly used.
...
18. **[SO #44]** Model Routing v1 — two dispatch modes via `DISPATCH_MODE` env on bridge plist. **`pure-codex`** *(active default, 6 May 2026)* — every dispatched brief routes to `codex --profile xhigh` regardless of declared agent; Agent field ignored at executor level. **`hybrid`** *(inactive)* — Sonnet executes 70-80%, Opus Max judges, Codex reviews; Agent field drives routing. Cowork orchestration sessions (LEAD/AUDITOR/COO/NARRATIVE) always run on Claude regardless of mode. Toggle: edit `EnvironmentVariables` in `~/Library/LaunchAgents/com.mzansiedge.cmux-bridge.plist` + `launchctl kickstart -k gui/$(id -u)/com.mzansiedge.cmux-bridge`. Codex Review Gate (SO #45) mandatory in both modes. Canonical: [Notion](https://www.notion.so/354d9048d73c8138bf72d8ce7b768a08) · mirror: [`ops/MODEL-ROUTING.md`](ops/MODEL-ROUTING.md). (LOCKED 2 May 2026; pure-codex activated 6 May 2026 `OPS-PURE-CODEX-ACTIVATE-01`.)
19. **[SO #45]** Review Gate — every code-touching brief runs a fresh-context review after commit + push and before `mark_done.sh`. **Hybrid mode (Claude executor):** `/codex:review --wait` slash command — fires the codex-companion script as a fresh node subprocess, providing cross-process review. **Pure-codex mode (Codex executor):** plugin path is REDUNDANT (executor is already Codex; calling its own plugin adds nothing and has hung agents on background-task UX). Instead, the agent spawns a **fresh `codex exec` sub-agent inline** — synchronous, no plugin, no shared session: `codex --profile xhigh exec "<review prompt>"`. New process = fresh context = unbiased sub-agent. Reports without `## Codex Sub-Agent Review` section showing `Outcome: clean | blockers-addressed | needs-changes | bootstrap-exempt` are INCOMPLETE — brief reopens. Mandatory adversarial triggers (money/payments, auth/settlement, non-rollback-safe migrations) → adversarial framing in the sub-agent prompt. Detail + canonical sub-agent prompt: `ops/DEV-STANDARDS.md` § Codex Review Gate v4.5 + § Pure-Codex Sub-Agent Review pattern. (LOCKED 3 May 2026 v4.4; amended 4 May 2026 v4.5; pure-codex inline-sub-agent rewrite 6 May 2026 `FIX-SO45-CODEX-INLINE-SUBAGENT-01`.)
...
21. **[SO #52]** Production runtime path isolation — `bot-prod/` is the read-only prod checkout at the deploy SHA; `bot/` is the dev-edit tree. Edits in `bot/` do NOT affect the live service until `deploy_bot_prod.sh <SHA>` runs. Never write directly to `bot-prod/`. Writable volume is `bot-data-shared/` only (DBs, reports, logs, bytecode). Drift watchman: `scripts/bot_tree_drift_check.sh` fires against `bot/`. Detail: `ops/RUNTIME-ISOLATION.md`. (LOCKED 7 May 2026.)
