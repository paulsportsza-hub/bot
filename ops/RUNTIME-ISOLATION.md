...
**CI invocation:** `.github/workflows/deploy-on-main.yml` SSHes the
prod host on every push to `main` and runs the script from the DEV
tree (`/home/paulsportsza/bot/scripts/deploy_bot_prod.sh`). The script
must always run from a writable location to avoid a bootstrap problem
