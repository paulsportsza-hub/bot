# LEAD Onboarding — Dispatch V2

*Paste-ready message for the LEAD Cowork session. Last updated: 2026-04-30.*
...
## Your enqueue command (LEAD role)
...
- **`sequential`** — use when this brief and any other currently in-flight brief
  touch the **same git repo** (e.g. both in `bot/`, or both in `publisher/`).
  This is the safe default. Wrong choice here causes pre-merge gate collisions.
- **`parallel`** — use only when this brief targets a **completely different
  git repo** from every other brief currently in `pending/`, `ready/`, or
  `running/`.
...
- You no longer `mv` YAML files between `queue/pending/`, `queue/ready/`, or
  `queue/running/` on the server.
- You no longer `ssh` into the server to run `dispatch_runner.sh` directly.
...
workspace title format, troubleshooting playbook (bridge silent-spawn,
surface.list parser, focus-steal, SSH key path, etc.).
