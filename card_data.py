...
#   5. Risk section ≥ 100 chars AND ≥ 1 specific risk noun from the 25-token
#      curated list (injury, rotation, fatigue, etc.).
#
...
# CTA + "↩️ Back" remain.
#
# This deliberately narrows the FIX-AI-BREAKDOWN-BUTTON-GATE-COVERAGE-01
# (Rule 20) mechanical-consistency invariant: the button-gate is no longer a
# strict superset of the synthesis predicate. Synthesis-on-tap (Rule 20) still
# WORKS for cache misses, but a w82-baseline-only cache row no longer gets a
# button. Per Paul: vague generic prose breaking the image-only rule is worse
# than no breakdown.

# Section quality thresholds — locked from brief AC-3.
...
# still in flight (Rule 23 Wave 2 chain — Sonnet retry → Haiku → defer).
...
# spellings observed in pregen output. Keep aligned with the bookmaker
# realignment helper in `bot.py` and `evidence_pack.py`.
...
# Decimal 1.36-99.99, fraction 11/10, American +150 / -200.
_BREAKDOWN_ODDS_RE = _re.compile(
    r"(?:\b[1-9]\d?\.\d{2}\b|\b\d+/\d+\b|(?:^|\s)[+-]\d{2,4}\b)"
)

# Curated risk-noun list. Brief AC-3 specifies "≥ 25 risk-noun tokens" —
# this list is 30 tokens covering soccer / rugby / cricket / combat /
# generic match-day risk vocabulary. Keep alphabetised within categories
# for ease of audit.
...
# Compile a single case-insensitive regex over the risk-noun list with
# longest-first ordering so multi-word phrases match before their bare prefix.
...
# Named-entity heuristic patterns. The brief specifies "team names, manager
# names from coaches.json, OR specific numbers ending in points / goals /
# wins / losses / meetings". The team/manager check is a substring match
# against evidence_pack-derived names; the numeric check is a regex.
...
def _section_extract_for_quality(narrative_html: str, section_key: str) -> str:
    ...
    # Strip the first line (section header).
...
def _count_setup_named_entities(
    ...
    Per brief AC-3: "team names, manager names from coaches.json, OR specific
    numbers ending in points, goals, wins, losses, meetings". The count is a
    UNIQUE-entity count, not a hit count — saying "Liverpool" three times in
    Setup counts as one entity, not three.

    Parameters
    ----------
    setup_text
        Plain text of the Setup section (HTML stripped).
    home_team / away_team
        Display names from evidence_pack. Empty strings skip team match.
    manager_names
        Tuple of manager / coach surnames from coaches.json. Empty tuple
        skips manager match.

    Returns
    -------
    int
        Distinct named-entity count. The brief threshold is 3.
    """
    if not setup_text:
        return 0
    ...
    # Team names (whole-word for single-word names; substring for multi-word).
    ...
    # Specific numeric entities (e.g. "5 wins", "12 points", "3 meetings").
...
def compute_breakdown_visibility(
    ...
    evidence_pack
        ...
        ``coaches`` (list/tuple of manager surnames). When ``None`` the
        named-entity check still works against numeric entities only.
    ...
    Notes
    -----
    Pure function — no DB access. The caller (``bot.py`` keyboard builder)
    is responsible for fetching the cache row and threading it through.
    A wrapper at the keyboard-builder call site queries the DB and feeds
    the row in.

    Logging is intentionally suppressed at this layer — the caller logs the
    visibility decision once per render with the failed condition list, so
    we don't double-log on every check.
    """
...
def compute_breakdown_visibility_reasons(
    ...
    Empty list ⇔ ``compute_breakdown_visibility`` returns True. Used by the
    bot.py keyboard builder for one-shot logging at render time, and by the
    contract tests for assertion-friendly error messages.
    """
    failures: list[str] = []
    if not isinstance(narrative_cache_row, dict):
        return ["row_not_dict"]
