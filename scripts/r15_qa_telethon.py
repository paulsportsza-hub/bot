#!/usr/bin/env python3
"""R15-QA-01: Opus Live Bot Scoring — Post Outcome Alignment Fix.
...
Key checks:
  ...
  - EV consistency: list EV vs detail EV
  - Tier consistency: list tier vs detail tier
  - Enrichment rate: genuinely enriched / total
  - Hallucination check
"""
...
RAW_CAPTURES_FILE = BOT_ROOT.parent / "reports" / "r15-qa-01-raw-captures.txt"
OUTPUT_DIR = BOT_ROOT.parent / "reports" / "r15-qa-captures"
...
def extract_list_outcome(btn_text: str) -> str:
    """Extract the team/outcome from the list button text."""
    # Button format: "💎 ARS vs CHE" or "🥇 3-letter vs 3-letter" or team names
...
def extract_list_ev(page_text: str, btn_text: str) -> str:
    """Extract EV for a specific card from the list page text."""
    # List format: [N] ⚽ Home vs Away 🥇\n     🏆 League\n     💰 Outcome @ odds (BK) · EV +X.X%
    # Find the EV line associated with this card
    clean_btn = btn_text.strip()
    for emoji in TIER_EMOJIS:
        clean_btn = clean_btn.replace(emoji, "").strip()
    clean_btn = clean_btn.replace("🔒", "").strip()

    lines = page_text.split("\n")
    for i, line in enumerate(lines):
        # Look for lines with EV near this card
...
def extract_detail_outcome(text: str) -> str:
    """Extract the recommended outcome from detail view."""
    # Look for the verdict/recommendation
    # Patterns: "Back Home @", "Back Away @", outcome in CTA
    ...
    verdict_match = re.search(r'🏆.*?(?:Back|back)\s+(.+?)(?:\.|,|\n|$)', text, re.I)
    if verdict_match:
...
def extract_detail_match_teams(text: str) -> tuple:
    """Extract home and away teams from detail view."""
...
def extract_list_match_teams(page_text: str, card_index: int) -> tuple:
    """Extract teams for a specific card from the list text."""
    # Format: [N] ⚽ Home vs Away 🥇
    pattern = rf'\[{card_index}\]\s*[⚽🏉🏏🥊]\s*(.*?)\s+vs\s+(.*?)(?:\s*[💎🥇🥈🥉🔒]|\s*$)'
    match = re.search(pattern, page_text, re.M)
    if match:
...
def analyse_card(text: str, cb_buttons: list, url_buttons: list) -> dict:
    analysis = {
        ...
        "has_edge": "🎯" in text and "Edge" in text.lower(),
    ...
    # Claimed facts for hallucination check
    facts = []
    ...
    # Extract outcome and EV from detail
...
async def main():
    print("=" * 70)
    print("R15-QA-01: Opus Live Bot Scoring — Post Outcome Alignment Fix")
    print(f"Date: {datetime.now().isoformat()}")
    print(f"Method: LIVE BOT INTERACTION via Telethon — NOT database reads")
    print(f"Model: Opus — 04 QA Surface A")
    print("=" * 70)

    raw_captures = []  # Collect ALL raw text for the file
    ...
    for i, (page_num, btn_text, btn_data, list_tier) in enumerate(all_card_buttons, 1):
        ...
        # Extract list-level info
        list_outcome = extract_list_outcome(btn_text)

        # Find EV from page text
        page_text = page_texts.get(page_num, "")
        list_ev = ""
        # Parse the card's EV from page text more carefully
        # Find the card number in the page and extract its EV line
        card_num_in_page = None
        lines = page_text.split("\n")
        for li, line in enumerate(lines):
            # Match [N] pattern
            num_match = re.search(r'\[(\d+)\]', line)
            if num_match and ("vs" in line or match_key.split("_")[0] in line.lower()):
                card_num_in_page = int(num_match.group(1))
                # Look for EV in next few lines
                for j in range(li, min(li + 5, len(lines))):
                    ev_m = re.search(r'EV\s*\+?([\d.]+)%', lines[j])
                    if ev_m:
                        list_ev = f"+{ev_m.group(1)}%"
                        break
                # Also extract list outcome from the money line
                for j in range(li, min(li + 5, len(lines))):
                    outcome_m = re.search(r'💰\s*(.*?)\s+@', lines[j])
                    if outcome_m:
                        list_outcome = outcome_m.group(1).strip()
                        break
                break
        ...
        # Parse teams
        home, away = extract_detail_match_teams(text)
        if home:
        ...
        print(f"    Sections: {qa['section_count']}/4 (Setup={qa['has_setup']} Edge={'🎯' in text} Risk={qa['has_risk']} Verdict={qa['has_verdict']})")
    ...
    out_file = OUTPUT_DIR / "r15_qa_results.json"
