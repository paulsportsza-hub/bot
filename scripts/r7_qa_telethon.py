#!/usr/bin/env python3
"""R7-QA-01: Formal Scored Telethon QA Audit.
...
OUTPUT_DIR = BOT_ROOT.parent / "reports" / "r7-qa-captures"
...
def extract_list_tier(btn_text: str) -> str:
    """Extract tier from list-view button text (e.g. '💎 SUN vs SEK')."""
    for emoji, tier in TIER_EMOJIS.items():
        if emoji in btn_text:
            return tier
    # Check for lock emoji (locked card)
    if "🔒" in btn_text:
        return "locked"
    return "unknown"
...
def extract_detail_tier(text: str, cb_buttons: list, url_buttons: list) -> str:
    """Extract tier from detail view CTA button or narrative."""
    # Check CTA button first
...
def analyse_card(text: str, cb_buttons: list, url_buttons: list) -> dict:
    """Extract QA-relevant fields from a detail card."""
    analysis = {
    ...
    # CTA button (URL button with bet/back in text)
    for btn in url_buttons:
        t = btn.get("text", "")
...
async def main():
    print("=" * 60)
    print("R7-QA-01: Formal Scored Telethon QA — @mzansiedge_bot")
    ...
    for i, (page_num, btn_text, btn_data, list_tier) in enumerate(all_card_buttons, 1):
        ...
        print(f"    List tier: {list_tier} | Detail tier: {detail_tier} | Match: {card_data['tier_match']}")
        print(f"    EV: {qa['ev_pct']}% | Prob: {qa['fair_prob_pct']}%")
        print(f"    MODEL ONLY: {qa['has_model_only']} | Sharp ref: {qa['has_sharp_reference']}")
        print(f"    Staking: {qa['staking_language']} | Compare: {qa['has_compare_odds']}")
        print(f"    CTA: {qa['cta_bookmaker_text']}")
        if qa['cta_url']:
            print(f"    CTA URL domain: {qa['cta_url_domain']}")
